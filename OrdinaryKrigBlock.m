...
%% Ordinary Kriging for Block Estimation
x = xcord;
y = ycord;
z = h;
blocksize = 200;
...
% Re-arranging the left side matrix for ordinary block estimate
n = length(h);
CovSamPoint(:,n+1) = 1;
CovSamPoint(n+1,:) = 1;
...
for ii=1:length(Xg)
    ...
    for jj=1:length(XgB)
        ...
        Lamda=lamda(:,ii);
        Lamda(numel(h))=[];
        H_Krig(ii,1) = sum(Lamda'*h);
        Sigma2E(ii,1) = sill-(sum(lamda(1:n,1).*CovSamUnsam(1:n,1))-lamda(n+1,1));
...
ylim([0 10000])
xlim([5000 15000])
title('\bfOrdinary block kriging map for Flow Unit 5 gross thickness')
xlabel('\bfX, ft')
ylabel('\bfY, ft')
colormap(jet)
colorbar
...
ylim([0 10000])
xlim([5000 15000])
title('\bfOrdinary block kriging variance map for Flow Unit 5 gross thickness')
xlabel('\bfX, ft')
ylabel('\bfY, ft')
colormap(jet)
colorbar
